...
| `CLAUDE.md` | Root operating manual. Filled by `/onboard`. Edit when your role/voice changes. Claude Code reads this automatically — root + any nested copies. |
...
| `scripts/` | You write Python or Bash to hit APIs not covered by MCPs | Most people's second connection is a script, not an MCP — scripts also pair naturally with hooks and scheduled runs for the Cadence layer |
| Hooks, scheduled agents, or cron + `claude -p` | You want something to run on a schedule without you asking | Claude Code has several Cadence options: hooks in `.claude/settings.json`, scheduled cloud agents (routines), or a cron job calling `claude -p "<prompt>"` non-interactively. Pick the lightest one that closes the gap. |
| Sub-OS folders (e.g. `youtube-os/`) | You have a vertical with its own data, sheets, transcripts, scripts | Isolation pattern — vertical workflows get their own scoped operating manual + skills (a nested `CLAUDE.md` works here) |
...
- `decisions/log.md` — every meaningful decision (`level-up.md` Phase 2 captures these automatically)
...
- **Don't fork your operating manual.** One `CLAUDE.md` at the root. Sub-OS folders can have their own scoped nested `CLAUDE.md`, but the root is canonical.
- **New capability = new skill.** Create `.claude/skills/<name>/SKILL.md` and it's auto-discovered as `/name` next session — no registration step. Keep skills in the repo so they version with everything else.
